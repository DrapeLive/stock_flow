...
class OrderRepo {
  ...
  Future<String> startEdit(int orderId) async {
    try {
      final res = await ApiClient.dio.post<Map<String, dynamic>>(
        '/api/orders/$orderId/start-edit/',
      );
      await _invalidateOrders();
      final data = res.data ?? const <String, dynamic>{};
      var status = _statusOf(data['status']);
      if (status == null) {
        final nested = data['order'];
        if (nested is Map) status = _statusOf(nested['status']);
      }
      status ??= _statusOf(data['order_status']);
      status ??= _statusOf((await getOne(orderId)).status);
      if (status != 'EDITING') {
        throw ApiException(
          'Could not start editing order #$orderId. '
          'The order is still ${status ?? 'in an unknown state'}.',
  ...
  /// Adds a scanned variant to a DRAFT or EDITING order (mirrors
  /// `orderApi.addItem`).
  Future<void> addItem(
    int orderId, {
