...
Future<void> _completePacking() async {
  ...
  Future<void> _openEdit() async {
    ...
    // The scanner/search item-picker screens add items through the shared
    // draft session, so the order must be claimed as EDITING on the server
    ...
    try {
      if (order.status == 'DRAFT') {
        ...
        final status = await repos.order.startEdit(order.id);
        if (status != 'EDITING') {
          throw ApiException('start-edit returned $status, expected EDITING');
        }
        OrderDraftSession.start(
          orderId: order.id,
          customerId: order.customer.id,
