...
const COLUMNS: ColumnDef[] = [
  { key: "name", label: "Customer Name", required: true, placeholder: "John Doe" },
  { key: "address", label: "Address", required: true, placeholder: "123 Main St…" },
  { key: "contact", label: "Contact", required: true, placeholder: "+91 98765 43210" },
  { key: "agent", label: "Agent", required: true, placeholder: "agent username" },
  { key: "gst", label: "GST No.", required: false, placeholder: "22AAAAA0000A1Z5" },
];

const uid = (): string => Math.random().toString(36).slice(2, 9);
const emptyRow = (): CustomerRow => ({
  _id: uid(), name: "", address: "", contact: "", agent: "", gst: "",
});

function normalizeRow(raw: Record<string, unknown>): BulkCustomerRequest {
  const lower: Record<string, string> = {};
  Object.keys(raw).forEach((k) => (lower[k.toLowerCase().trim()] = String(raw[k] ?? "")));
  return {
...
function downloadTemplate() {
  const sample = [
    { "Customer Name": "John Doe", Address: "123 Main St, Chennai", Contact: "+91 98765 43210", Agent: "agent_username", "GST No.": "22AAAAA0000A1Z5" },
    { "Customer Name": "Jane Smith", Address: "456 Park Ave, Mumbai", Contact: "+91 91234 56789", Agent: "agent_username", "GST No.": "" },
  ];
  const ws = XLSX.utils.json_to_sheet(sample);
  // Column widths
  ws["!cols"] = [{ wch: 20 }, { wch: 30 }, { wch: 18 }, { wch: 18 }, { wch: 18 }];
    ...
    <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-lg overflow-hidden">
      ...
      <div className="px-6 pb-4 grid grid-cols-2 gap-3">
        <div className="bg-green-50 border border-green-100 rounded-2xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-green-500">Saved successfully</p>
          </div>
        </div>
        <div className="bg-rose-50 border border-rose-100 rounded-2xl px-4 py-3 flex items-center gap-3">
          <AlertCircle size={20} className="text-rose-400 shrink-0" />
          <div>
            <p className="text-xl font-black text-rose-600">{backendErrors.length}</p>
      ...
      {backendErrors.length > 0 && (
        <div className="px-6 pb-4">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Failed Rows</p>
          <div className="max-h-52 overflow-y-auto space-y-2 pr-1">
            {backendErrors.map((e, i) => (
              <div key={i} className="flex items-start gap-2 bg-rose-50 border border-rose-100 rounded-2xl px-3 py-2">
                <AlertCircle size={14} className="text-rose-400 mt-0.5 shrink-0" />
                <div className="text-xs">
                  <span className="font-black text-rose-600">Row {e.row}</span>
                  {e.name && <span className="text-gray-500 ml-1">· {e.name}</span>}
      ...
      <div className="px-6 pb-6 flex gap-2 justify-end">
        {backendErrors.length > 0 && (
          <button
            onClick={() => { onRemoveFailed(); onClose(); }}
...
export default function BulkImportPage(): React.ReactElement {
  ...
  const parseFile = useCallback((file: File | null | undefined) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsed: CustomerRow[] = json.map((r) => ({ _id: uid(), ...normalizeRow(r) }));
        setRows(parsed);
  ...
  const updateCell = (id: string, col: keyof BulkCustomerRequest, val: string) => {
    setRows((prev) => {
      const next = prev.map((r) => r._id === id ? { ...r, [col]: val, _backendError: undefined } : r);
  ...
  const deleteSelected = () => { setRows((p) => p.filter((r) => !selected.has(r._id))); setSelected(new Set()); };
  const addRow = () => setRows((p) => [...p, emptyRow()]);
  const toggleSelect = (id: string) => setSelected((s) => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleAll = () => setSelected(selected.size === rows.length ? new Set() : new Set(rows.map((r) => r._id)));
  const clearAll = () => { setRows([]); setFileName(""); setStatus("idle"); setErrors({}); setSelected(new Set()); setBackendErrors([]); };
  ...
  const handleConfirm = async () => {
    ...
    if (Object.keys(errs).length) {
      setErrors(errs);
      showToast(`Fix ${Object.keys(errs).length} invalid row(s) before saving`, "error");
      return;
    }
    setStatus("saving");
    const payload: BulkCustomerRequest[] = rows.map(({ _id, _backendError, ...rest }) => rest);
    try {
      ...
      if (bErrs.length) {
        ...
        setRows((prev) =>
          prev.map((r) =>
            errByName.has(r.name) ? { ...r, _backendError: errByName.get(r.name) } : r
          )
        );
  ...
  return (
    <div className="min-h-screen py-10 px-6">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-5 right-5 z-50 max-w-sm px-5 py-4 rounded-2xl border shadow-lg text-sm font-semibold flex items-center gap-2
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-black text-gray-900 leading-tight">Bulk Import</h1>
        ...
        <div
          ...
          className={`bg-white rounded-3xl border-2 border-dashed cursor-pointer transition-all duration-200 text-center mb-6 shadow-sm
            ${dragOver ? "border-primary bg-primary/5"
              : fileName ? "border-gray-200 hover:border-primary/40"
        ...
        >
          <input
            ...
            accept=".xlsx,.xls,.csv"
            className="hidden"
            onChange={(e: ChangeEvent<HTMLInputElement>) => parseFile(e.target.files?.[0])}
          />
          {fileName ? (
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-gray-400 text-xs mt-0.5">{rows.length} rows loaded · Click to replace</p>
                </div>
              </div>
              <button
            ...
            <>
              ...
              <p className="text-base font-bold text-gray-700">Drop your spreadsheet here</p>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap mb-4">
              <div className="flex items-center gap-2 text-sm">
                <span className="text-gray-500 font-bold">{rows.length} rows</span>
                {selected.size > 0 && (
              ...
              <div className="ml-auto flex items-center gap-2 flex-wrap">
                ...
                >
                  {status === "saving" ? "Saving…" : errorCount > 0 ? `Fix ${errorCount} error(s)` : `Save ${rows.length} Customers`}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      ...
                      <th className="px-3 py-3.5 w-10 text-left text-gray-300 text-xs font-black tracking-widest uppercase">#</th>
                      {COLUMNS.map((col) => (
                        <th key={col.key} className="px-3 py-3.5 text-left text-xs font-black tracking-widest uppercase text-gray-400 whitespace-nowrap">
                          {col.label}{col.required && <span className="text-primary ml-1">*</span>}
                        </th>
                      ))}
                      <th className="px-4 py-3.5 w-16 text-center text-xs font-black tracking-widest uppercase text-gray-300">Status</th>
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-2 text-center">
                            <input type="checkbox" checked={selected.has(row._id)} onChange={() => toggleSelect(row._id)} className="w-4 h-4 rounded accent-primary cursor-pointer" />
                          </td>
                          <td className="px-3 py-2 text-gray-300 text-xs font-bold select-none">{idx + 1}</td>
                          {COLUMNS.map((col) => (
              ...
              <div className="border-t border-gray-100 px-5 py-3 flex items-center justify-between text-xs text-gray-400 font-semibold">
                <span>Fields marked <span className="text-primary font-black">*</span> are required · Click any cell to edit</span>
            ...
            <div className="mt-5 flex justify-end items-center gap-3 px-1">
              ...
              >
                {status === "saving" ? "Saving…" : errorCount > 0 ? `Fix ${errorCount} error(s) first` : `✓ Save ${rows.length} Customers`}
              </button>
            </div>
          </div>
        )}
        ...
        {rows.length === 0 && status === "idle" && (
          <div className="mt-10 text-center space-y-2">
            ...
            <p className="text-gray-400 font-bold text-sm mt-3">Upload a spreadsheet to get started</p>
          </div>
        )}
          ...
          <div className="mt-3 bg-white border border-gray-100 rounded-3xl p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 shadow-sm">
            {[
              ...
              <div key={m.field} className="text-xs flex items-center gap-2">
                <code className="text-primary bg-primary/8 border border-primary/10 rounded-lg px-2 py-1 font-mono text-[11px]">{m.sheet}</code>
