    ...
    <div className="min-h-screen bg-gray-50/50 pb-32">
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-xl font-black text-gray-900 leading-tight">Order Details</h1>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Step 2: Add Items</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-6 pt-8">
        ...
        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4 mb-8">
          ...
          <div>
            <p className="text-[10px] text-gray-400 uppercase font-black tracking-wider leading-none mb-1">Customer</p>
            <h3 className="text-lg font-bold text-gray-900 leading-tight">{data?.name}</h3>
            {data?.address && <p className="text-xs text-gray-400 mt-0.5">{data.address}</p>}
          </div>
        </div>
        ...
        <div className="flex justify-between items-end mb-6">
          <div className="flex flex-col">
            ...
            <div className="flex gap-2 items-center mt-1">
              <span className="text-gray-400 text-[10px] font-bold uppercase tracking-wider">Selected</span>
              <div className="bg-amber-100 text-amber-600 rounded-full py-0.5 px-3 border border-amber-200">
                <span className="font-bold text-xs">{orders?.items.length || 0}</span>
              </div>
            </div>
          </div>
          <StockFlowButton
        ...
        <div className="space-y-4">
          {!orders || orders.items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 bg-white rounded-3xl border border-dashed border-gray-200">
              ...
              <p className="text-[10px] text-gray-300 uppercase tracking-widest mt-1">Scan QR codes to add products</p>
            </div>
          ) : (
